...
namespace All_Error_Solver
{
    class Contacts_Class
    {
        ...
        public Contacts_Class(DataRow row)
        {
            ...
            Phone_number = Convert.ToInt32(row["Phone_number"]);
        }

        public static List<Contacts_Class> select()
        {
            DataTable dt = DB.select("SELECT * FROM `contacts` ", new List<DbParameter>());
        ...
        public void delete()
        {
            DB.select("DELETE FROM `contacts` WHERE `id` = @id", new List<DbParameter>() { new DbParameter { name = "@id", value = id } });
        }

        public static void add(string FIO, string Position, string Mail, int Phone_number)
        {
            DB.select("INSERT INTO `contacts` (`FIO`, `Position`, `Mail`, `Phone_number`) VALUES (@FIO, @Position, @Mail, @Phone_number);",
        ...
        {
            DB.select("UPDATE `contacts` SET `id` = @id, `FIO` = @FIO, `Position` = @Position, `Mail` = @Mail, `Phone_number` = @Phone_number WHERE `contacts`.`id` = @id; ",
        ...
        {
            DataTable dt = DB.select($"SELECT * FROM contacts WHERE {searchParam} LIKE '%{searchValue}%';", new List<DbParameter>());
            List<Contacts_Class> cc = new List<Contacts_Class>();
